...
Nguồn công bố: [SBA 7(a) & 504 FOIA](https://data.sba.gov/dataset/7a-504-foia).
Khi tải lại, trang có thể đã đổi snapshot; dùng tên file và checksum trong `docs/data_profile.json` để đối chiếu.
CSV lớn được quản lý qua Git LFS (cần chạy `git lfs install` và `git lfs pull` khi clone). Khi nộp bài, copy cả CSV và XLSX vào thư mục `Data/`.
